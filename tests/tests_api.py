...
def test_api_posts_by_id(posts_keys):
    ...
    posts = response.json
    assert isinstance(posts, dict), "Есть ошибка получения поста по ID: выгружается не список"
    assert set(posts[0].keys()) == posts_keys, "Ошибка получения ключей при загрузке ID"
